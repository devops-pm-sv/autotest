...
class PATH:
    ...
    config_tmp_path = "E:\\PyProject\\autotest\\report\\BeautifulReport\\template"
...
class ReportTestResult(unittest.TestResult):
    ...
    def startTest(self, test) -> None:
        ...
        :return:
        """
        LogSet.info(msg=f"用例  [ {test} ]  开始执行...")
    ...
    @classmethod
    def str_err(self,err):
        """
            获取sys.exc_info()的参数并返回字符串类型的数据
        """
        err_list = traceback.format_exception(*err)
        error = ""
        for i in err_list:
            error += i
        return error
    ...
    def addSuccess(self, test) -> None:
        ...
        :return:
        """
        LogSet.info(msg=f"用例  [ {test} ]  执行成功!\n\n\n")
        logs = []
        output = self.complete_output()
        logs.append(output)
        sys.stdout.write(output) # 修改
        # if self.verbosity > 1:
        #     sys.stderr.write('ok ')
    ...
    def addError(self, test, err):
        ...
        """
        LogSet.error(msg=f"用例  [ {test} ]  执行失败：\n{self.str_err(err)}\n\n\n")
        logs = []
        output = self.complete_output()
    ...
    def addSkip(self, test, reason) -> None:
        ...
        logs = [reason]
        self.complete_output()
        LogSet.debug(msg=f"用例  [ {test} ]  跳过... Skip Of Reason：{reason}\n\n\n")
        self.skipped += 1
...
class BeautifulReport(ReportTestResult, PATH):
    ...
    def report(self, description, filename: str = None, log_path='.'):
        ...
        self.output_report()
        sys.stdout.write(f""
                         f"Total：{self.success_count+self.error_count+self.failure_count+self.skipped}  "
                         f"Pass：{self.success_count}  "
                         f"Fail：{self.error_count+self.failure_count}  "
                         f"Skip：{self.skipped}")
        sys.stdout.write(f'\n\n测试已ok, 报告位置 {self.log_path}')
